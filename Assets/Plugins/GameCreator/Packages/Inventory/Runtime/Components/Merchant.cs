...
namespace GameCreator.Runtime.Inventory
{
    ...
    public class Merchant : MonoBehaviour
    {
        ...
        [SerializeField] private PropertyGetDecimal m_BuyRate = GetDecimalPercentage.Create(1f);
        [SerializeField] private PropertyGetDecimal m_SellRate = GetDecimalPercentage.Create(0.5f);
        ...
        public bool CanBuyFromClient(Bag clientBag, RuntimeItem runtimeItem)
        {
            ...
            if (!this.m_InfiniteCurrency)
            {
                ...
                if (merchantBag.Wealth.Get(item.Price.Currency.ID) < price) return false;
            }
        ...
        public bool SellToClient(Bag clientBag, RuntimeItem runtimeItem)
        {
            ...
            IdString currencyID = runtimeItem.Item.Price.Currency.ID;
            int price = this.GetBuyPrice(runtimeItem, clientBag);

            clientBag.Wealth.Subtract(currencyID, price);
            if (!this.m_InfiniteCurrency) merchantBag.Wealth.Add(currencyID, price);
